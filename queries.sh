...
echo "$($PSQL "SELECT Distinct(name) FROM teams inner join games ON teams.team_id = games.winner_id or teams.team_id = games.opponent_id where name ~ '^Co' order by name asc")"
